	...
	--=========================================================
		...
		ATOMClient.Patcher = {
			...
			---------------------------------------------------------
			-- Add
			Add = function(self, e, f, p, m, n, onlyGlobal)
				--Msg(0, "added for %s", e)
			...
			FAdd = function(self, e, f, p, m, n, onlyGlobal)
				...
				if (ok) then
					...
					self:DoHook(ent, f, t, p, doCall, onlyGlobal, is_object, ok);
				end;
				--self.toHook[ #(self.toHook) + 1 ] = { e, f, p, m, n, onlyGlobal };
			end,
			---------------------------------------------------------
			...
			DoHook = function(self, ent, f, t, p, doCall, onlyGlobal, is_object, ok)
				--Msg(0, tostring(f))
				local classEnts = System.GetEntitiesByClass(ent);
		...
		ATOMClient.OnLeaveVehicleSeat = function(self, vehicle, seat, passengerId, exiting)
			local passenger = System.GetEntity(passengerId);
			if (not passenger) then
				return;
			end
			if (exiting) then
				--local item = passenger.inventory:GetCurrentItem();
				--if (item and item.CM) then
				passenger.ICMId = nil; -- refresh possible item models
				passenger.ICML = nil; -- refresh possible item models
				--end;
			end
			
			Msg(1, "id=%s,driver=%s,jet=%s,thruster=%s,power=%f/0", tostring(passengerId == g_localActorId), tostring(seat.seat:IsDriver()), tostring(vehicle.IsJet), tostring(vehicle.ThrusterON), (vehicle.ThrusterPowerVisual or 0))
			if (passengerId == g_localActorId and seat.seat:IsDriver() and vehicle.IsJet and vehicle.ThrusterON and (vehicle.ThrusterPower or 0)>0) then
		...
		ATOMClient.OnAction = function(self, action, activation, value)
			...
			elseif (action == "nextitem") then --action == "small" or action == "medium") then
				...
				for i, v in pairs(all or{}) do
					if (System.GetEntity(v).weapon) then
						total = total + 1;
					end;
				end;
		...
		ATOMClient.HookGameRules = function(self)
			...
			self.Patcher:Add("g_gameRules", function(self, p, s, w, dmg, m, t)
				...
				if (p == g_localActorId) then
					...
					g_gameRules.FadeScreen(g_gameRules);
				end
			...
			-- OnDisconnect
			---------------------
			self.Patcher:Add("g_gameRules", function(self, cause, desc)
		...
		ATOMClient.AddHitMarker = function(self, hier, entfernung)
			...
			if (entfernung < 80) then
				...
				HIT_MARKERS[ #HIT_MARKERS+1 ] = { dort = hier, zeit = _time, lebensdauer = 1, expire = _time + 1 }
			end
		end
				...
				for i = 1, #HIT_MARKERS do
					...
					if (marker and _time < marker.expire) then
						...
						if (entfernung <= 80) then
							...
							if (alpha > 0) then
								-- (entfernung / 100) * 10
								System.DrawLabel( marker.dort, 1.5, "$4(X)", 1, 0, 0, alpha );
							end
		...
		ATOMClient.OnHit = function(self, player, hit)	
			...
			if (ht == "lockpick" or hit == "repair") then
				return
			end
		
			local shooter_is_localActor = hit.shooterId == g_localActorId
			local target_is_localActor  = hit.targetId  == g_localActorId
			local is_self_hit           = shooter_is_localActor and target_is_localActor
			
			local is_bullet = ht:find("bullet")
			local is_melee = ht == "melee"
			local hs = g_gameRules:IsHeadShot(hit)
			local weapon_class = hit.weapon and hit.weapon.class
			
			if (shooter_is_localActor and not is_self_hit and is_bullet) then
				--local s = (armor or 100) > 10 and "armor" or "flesh";
				--s = "sounds/physics:bullet_impact:helmet_feedback"; --(hs and "sounds/physics:bullet_impact:headshot_feedback_fp" or CUSTOM_HIT_SOUNDS and "sounds/physics:bullet_impact:helmet_feedback"or"sounds/physics:bullet_impact:mat_" .. s .. "_fp")
			...
			if (is_bullet) then
				...
				if (hs) then
					...
					if (weapon_class == "DSG1" or weapon_class == "GaussRifle") then
						blood_scale = weapon_class == "DSG1" and 2 or 3
					end
		...
		ATOMClient.ToServer = function(self, t, p1, p2, p3, p4, p5, p6, p7, p8, ...)
			...
			elseif (t == eTS_Report) then
				...
				if (g_gameRules.class == "InstantAction") then
					g_game:RenamePlayer(g_localActorId, report)
				else
		...
		ATOMClient.UpdateJets = function(self, frameTime)
					...
					if (hit) then
						...
						if (entity) then
							...
							if (entity.vehicle) then
								...
								msg = "$9" .. teams[team] .. "$9 Vehicle $1" .. entity.class .. "$9 (HP : " .. (temp < 25 and "$4" or "$5") .. temp .. "$9)";
							...
							elseif (entity.actor) then
								...
								msg = "$9" .. teams[team] .. "$9 Player $1" .. entity:GetName() .. "$9 (Health : " .. (temp < 25 and "$4" or "$5") .. temp .. "$1, Eneryg : $5" .. entity.actor:GetNanoSuitEnergy() .. "$9, Suit : " .. suits[entity.actor:GetNanoSuitMode()] .. "$9)";
		...
		ATOMClient.CheckMovement = function(self, frameTime) -- base function from nCX client from ctaoistrach, modified by tyson 24/07/2021
			if (not g_localActor.last_movecheck or _time - g_localActor.last_movecheck >= 1) then
				if (not g_localActor.SuperSpeed) then
					...
					elseif (flying and speed == 0 and not g_localActor:Underwater()) then
						...
						local normSpeed = not flying and 13 or 20; -- walljumpers trigger false detections when flying
						...
						if (System.GetCVar("Log_verbosity")>=3 and (speed>0 or speed<0)) then -- so it wont spam at log verb 2
							System.Log("Speed: " .. speed .. " > " .. maxSpeed .. " > " .. tpSpeed .. " | " .. (g_localActor.speedDetection or 0));
						end;
					end;
					
					local stats = g_localActor:GetPhysicalStats();
					
					if (stats and g_localActor.actor:GetSpectatorMode() == 0 and not g_localActor.actor:GetLinkedVehicleId() and g_localActor.actor:GetHealth() > 0 and g_localActor.actor:GetPhysicalizationProfile() == "alive") then
						...
						if (ATOM_NO_DTS ~= true) then
							...
							elseif (flags ~= "1.92939e+008" and flags ~= "192939008" and flags~="1.84682e+008" and flags~="184682000" and flags~="1.84551e+008" and flags~="184551000" and flags~="1.84584e+008" and flags~="184550992") then
